...
int server_send_t2run(void)
{
	...
	return server_send_package(SERVER_TX_T2RUN, fields);
}

/*
Envio do pacote T2STOP: $T2STOP,RESULT
*/
int server_send_t2stop(void)
{
    char fields[32];
    int ret;
	...
	return server_send_package(SERVER_TX_T2STOP, fields);
}

/*
Envio do pacote T2TIMER: $T2TIMER,DELAY,RESULT
*/
int server_send_t2timer(int delay)
{
    char fields[32];
    int ret;

    ret = task2_timer(delay);

	sprintf(fields, "%d,%d", delay, ret);

	return server_send_package(SERVER_TX_T2TIMER, fields);
}
...
void server_on_cmd_received(int argc, char **argv)
{
    ...
    */
    ...
    $T2RUN
    */
    if (strcmp(cmd, SERVER_RX_T2RUN) == 0) {
    ...
    $T2STOP
    */
    if (strcmp(cmd, SERVER_RX_T2STOP) == 0) {
    ...
    $T2TIMER,DELAY
    */
    if (strcmp(cmd, SERVER_RX_T2TIMER) == 0) {
